...
namespace Projeto.Repositories
{
    public class SugestaoRepository : ISugestaoRepository
    {
        ...
        public async Task ExcluirComentarios(int sugId)
        {
            var votos = _context.Comentario.Where(uv => uv.SugestaoId == sugId).ToList();
            _context.Comentario.RemoveRange(votos);
            await _context.SaveChangesAsync();
        }

        public async Task<IEnumerable<Sugestao>> ListarSugestaoPorStatus(string status)
        {
            return await _context.Sugestao.Where(s => s.StatusSugestao == status).Include(s => s.Usuario).Include(s => s.Sugestao_Categorias).ThenInclude(sg => sg.Categoria).OrderByDescending(r => r.Votos).ToListAsync();
        }

        public async Task<IEnumerable<Sugestao>> ListarSugestaoPorLugar(string lugar)
        {
            return await _context.Sugestao.Where(s => s.Localizacao == lugar).Include(s => s.Usuario).Include(s => s.Sugestao_Categorias).ThenInclude(sg => sg.Categoria).OrderByDescending(r => r.Votos).ToListAsync();
        }
